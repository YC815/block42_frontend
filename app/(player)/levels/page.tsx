...
export default function LevelsPage() {
  ...
  const filterLevels = (levels: typeof officialQuery.data) => {
    if (!levels) return [];
  ...
  const officialLevels = useMemo(
    () => filterLevels(officialQuery.data),
    [officialQuery.data, effectiveFilter, progressMap]
  );

  const communityLevels = useMemo(
    () => filterLevels(communityQuery.data),
    [communityQuery.data, effectiveFilter, progressMap]
  );

  const activeStats = useMemo(() => {
    const levels = activeTab === "official" ? officialQuery.data : communityQuery.data;
    if (!levels) {
      ...
      <div className="mx-auto max-w-6xl space-y-6">
        ...
        <Tabs value={activeTab} onValueChange={handleTabChange}>
          ...
          <TabsContent value="official" className="mt-6">
            ...
            {officialQuery.data && officialQuery.data.length > 0 && officialLevels.length === 0 && (
          ...
          <TabsContent value="community" className="mt-6">
            ...
            {communityQuery.data && communityQuery.data.length > 0 && communityLevels.length === 0 && (
