...
export function ExecutionThreadBar({ queue, actions, embedded }: ExecutionThreadProps) {
  ...
  const containerClass = embedded
    ...
    : "border border-slate-900/10 bg-white/85 px-4 py-3 shadow-[0_16px_40px_-32px_rgba(15,23,42,0.5)] backdrop-blur";
        ...
        {hasQueue ? (
          queue.map((command, index) => (
            <div
              ...
              className={`flex h-9 min-w-[36px] items-center justify-center rounded-lg border text-xs font-semibold transition ${
                index === 0
            ...
            >
              <span
                className={`flex h-6 w-6 items-center justify-center rounded ${
                  COMMAND_ICONS[command.type].bg ?? ""
                } ${COMMAND_ICONS[command.type].bg ? "text-white" : ""}`}
              >
                {COMMAND_ICONS[command.type].label}
              </span>
